...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      className={`fixed left-0 right-0 z-50 w-full max-w-full transition-all duration-500 ${scrolled
            ...
            return (
              <div key={link.name} className="flex flex-col items-center relative">
                <a
                  ...
                  className={`relative text-[12px] font-bold tracking-widest uppercase transition-colors duration-300 ${isActive ? "!text-white" : "!text-white/70 hover:!text-white"
                    }`}
                >
                  {link.name}
                </a>
            ...
            <div className="px-8 py-6 flex flex-col gap-1">
              {NAV_LINKS.map((link, i) => (
                <motion.a
                  ...
                  className={`px-4 py-3 rounded-lg text-sm font-semibold tracking-wide uppercase transition-all [font-family:'Poppins',sans-serif] ${activeSection === link.href.replace("#", "")
